...
class _DialogTestPageState extends State<DialogTestPage> {
  ...
  Widget build(BuildContext context) {
    return JhTextList(
      ...
      callBack: (index, str) {
        if (str == 'JhDialog-标题') {
          LdgDialog.show(context, title: '提示', onConfirm: () {
            JhToast.showText(context, msg: '点击确定');
          });
        }
        if (str == 'JhDialog-标题不加粗') {
          LdgDialog.show(context, title: '提示', isBoldTitle: false);
        }
        if (str == 'JhDialog-标题内容') {
          LdgDialog.show(context, title: '提示', content: '您确定要退出登录吗？');
        }
        if (str == 'JhDialog-内容') {
          LdgDialog.show(context, content: '确认要退出吗？');
        }
        if (str == 'JhDialog-标题内容只有确定') {
          LdgDialog.show(context, title: '警告', content: '您的账号在异地登录，请重新登录！', rightText: '好的', hiddenCancel: true);
        }
        if (str == 'JhDialog-修改按钮文字') {
          LdgDialog.show(context, title: '提示', content: '您需要同意相关协议才能使用！', leftText: '不同意', rightText: '同意');
        }
        if (str == 'JhDialog-点击按钮弹框不消失') {
          LdgDialog.show(context, title: '提示', content: '点击取消按钮弹框消失，点击确认按钮延时3秒后弹框消失', clickBtnPop: false, onConfirm: () {
        ...
        if (str == 'JhDialog-拦截取消按钮点击事件') {
          LdgDialog.show(context, title: '提示', content: '点击取消按钮弹框不消失，点击确认按钮弹框消失', onConfirm: () {
            JhToast.showText(context, msg: '点击确定');
          }, onCancel: () {
          ...
          LdgDialog.showInputDialog(
            ...
            onConfirm: () {
              if (inputValue.isEmpty) {
                LdgProgressHUD.showText('Please input number');
                return;
              }
              // 手动隐藏弹框
              LdgDialog.hide(context);
              LdgProgressHUD.showText('number: $inputValue');
        ...
        if (str == 'JhDialog-自定义内容') {
          LdgDialog.showCustomDialog(context, content: Container(height: 200, color: Colors.red));
        }
        if (str == 'JhDialog-自定义内容2') {
          LdgDialog.showCustomDialog(context, title: '提示', content: Container(height: 200, color: Colors.red));
        }
  ...
  _showInputDialog() async {
    ...
    LdgDialog.showCustomDialog(
      ...
      onCancel: () => LdgDialog.hide(context),
      onConfirm: () {
        if (inputValue.isEmpty) {
          LdgProgressHUD.showText('Please input number');
          return;
  ...
  _jumpPage(params, isShowDialog) {
    if (_selectItem.isEmpty) {
      LdgProgressHUD.showText('请选择类型');
      return;
    }

    LdgDialog.hide(context);
    LdgProgressHUD.showText('验证通过，进行其他操作');
    print('params: $params');
  }
}
